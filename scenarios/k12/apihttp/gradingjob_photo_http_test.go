...
//  2. 轮询 GET /grading-jobs/{id} 到 awaiting_confirmation：响应携带识别停点产物
//     recognition（题目清单 + 整卷学科 + 锚点 bbox），供护栏回显。
//  3. POST /{id}/confirm 带结构化修正（subject/grade/question_corrections）→ 确认冻结
//     canonical 输入后异步续跑到 completed。
//  4. completed 后 GET 携带 result（逐题判定 verdict 五值 + 入库信息 + bbox），
...
func TestPhotoJobHTTP_DesktopFlowToCompleted(t *testing.T) {
	...
	// 停点产物：识别题目 + 整卷学科 + 锚点 bbox（护栏回显数据源）。
	...
	// 家长确认（含修正：作答 10.4 保留，走批改）。
	confirmBody := `{"agent":"mingming","subject":"数学","grade":"五年级上","question_corrections":[{"index":0,"student_answer":"10.4","answer_state":"present"}]}`
	...
	if result == nil {
		t.Fatalf("completed 响应应携带 result: %v", out)
	}
