...
// K12GradingPhysicalCallsV44 cuts the grading ledger over from one logical
// solve row to one row per physical provider request and atomically correlates
// every succeeded request with one unique cost receipt.
var K12GradingPhysicalCallsV44 = Migration{
	Version:     44,
	Description: "BUG-20260726 K12 批改物理调用账本与唯一费用回执",
	AtomicFunc:  migrateK12GradingPhysicalCallsV44,
}

const k12GradingPhysicalInvocationV44DDL = `
CREATE TABLE k12_grading_item_invocations_v44 (
    ...
    CHECK(status != 'succeeded' OR
        (cost_receipt_id != '' AND result_digest != '' AND result_json != '')),
    CHECK(status NOT IN ('failed','outcome_unknown') OR
        (failure_class != '' AND failure_code != ''))
);`

const k12GradingPhysicalAssessmentV44DDL = `
CREATE TABLE k12_grading_assessment_items_v44 (
    agent_name TEXT NOT NULL,
    job_id TEXT NOT NULL,
    problem_id TEXT NOT NULL,
    attempt_id TEXT NOT NULL,
    ...
    FOREIGN KEY(solve_invocation_id)
        REFERENCES k12_grading_item_invocations_v44(item_invocation_id),
...
const k12GradingPhysicalV44PostDDL = `
ALTER TABLE k12_grading_item_invocations_v44 RENAME TO k12_grading_item_invocations;
ALTER TABLE k12_grading_assessment_items_v44 RENAME TO k12_grading_assessment_items;
...
func migrateK12GradingPhysicalCallsV44(
	ctx context.Context,
	db *sql.DB,
	...
	if err != nil {
		return fmt.Errorf("检查 V44 分题调用账本: %w", err)
	}
	hasAssessments, err := tableExists(ctx, db, "k12_grading_assessment_items")
	if err != nil {
		return fmt.Errorf("检查 V44 批改回执: %w", err)
	}
	if !hasInvocations && !hasAssessments {
	...
	if !hasInvocations || !hasAssessments {
		return fmt.Errorf("V44 分题调用账本与批改回执结构不完整")
	}
	hasCostReceipt, err := columnExists(ctx, db, "k12_grading_item_invocations", "cost_receipt_id")
	if err != nil {
		return fmt.Errorf("检查 V44 cost_receipt_id: %w", err)
	}
	...
	if err := db.QueryRowContext(ctx, `SELECT sql FROM sqlite_master
		...
		return fmt.Errorf("读取 V44 分题调用 DDL: %w", err)
	}
	if hasCostReceipt && strings.Contains(invocationDDL, "'solve_generate'") {
		tx, txErr := db.BeginTx(ctx, nil)
		if txErr != nil {
			return txErr
		}
	...
	if err != nil {
		return fmt.Errorf("获取 V44 迁移连接: %w", err)
	}
	defer conn.Close()
	var foreignKeys int
	if err := conn.QueryRowContext(ctx, `PRAGMA foreign_keys`).Scan(&foreignKeys); err != nil {
		return fmt.Errorf("读取 V44 foreign_keys: %w", err)
	}
	if _, err := conn.ExecContext(ctx, `PRAGMA foreign_keys=OFF`); err != nil {
		return fmt.Errorf("关闭 V44 foreign_keys: %w", err)
	}
	defer func() {
		if foreignKeys != 0 {
			if _, err := conn.ExecContext(context.Background(), `PRAGMA foreign_keys=ON`); retErr == nil && err != nil {
				retErr = fmt.Errorf("恢复 V44 foreign_keys: %w", err)
			}
		}
	}()
	...
	if err != nil {
		return fmt.Errorf("开启 V44 事务: %w", err)
	}
	defer tx.Rollback()
	for _, table := range []string{
		"k12_grading_assessment_items_v44",
		"k12_grading_item_invocations_v44",
	} {
		if _, err := tx.ExecContext(ctx, `DROP TABLE IF EXISTS `+table); err != nil {
			return fmt.Errorf("清理 V44 暂存表 %s: %w", table, err)
		}
	}
	if _, err := tx.ExecContext(ctx, k12GradingPhysicalInvocationV44DDL); err != nil {
		return fmt.Errorf("创建 V44 分题调用账本: %w", err)
	}
	if _, err := tx.ExecContext(ctx, k12GradingPhysicalAssessmentV44DDL); err != nil {
		return fmt.Errorf("创建 V44 批改回执: %w", err)
	}
	if _, err := tx.ExecContext(ctx, `INSERT INTO k12_grading_item_invocations_v44 (
    ...
    SELECT item_invocation_id,agent_name,job_id,problem_id,attempt_id,
        operation,operation_attempt,request_digest,provider,model,route_snapshot_json,status,
        CASE WHEN status='succeeded' THEN 'cost-'||item_invocation_id ELSE '' END,
        result_digest,result_json,failure_class,failure_code,created_at,updated_at
    FROM k12_grading_item_invocations`); err != nil {
		return fmt.Errorf("复制 V44 分题调用证据: %w", err)
	}
	if _, err := tx.ExecContext(ctx, `INSERT INTO k12_grading_assessment_items_v44 (
    ...
    FROM k12_grading_assessment_items`); err != nil {
		return fmt.Errorf("复制 V44 批改回执证据: %w", err)
	}
	for _, statement := range []string{
		`DROP TABLE k12_grading_assessment_items`,
		`DROP TABLE k12_grading_item_invocations`,
		k12GradingPhysicalV44PostDDL,
	} {
		if _, err := tx.ExecContext(ctx, statement); err != nil {
			return fmt.Errorf("切换 V44 账本结构: %w", err)
		}
	}
	var violations int
	if err := tx.QueryRowContext(ctx, `SELECT COUNT(*) FROM pragma_foreign_key_check`).Scan(&violations); err != nil {
		return fmt.Errorf("检查 V44 外键: %w", err)
	}
	if violations != 0 {
		return fmt.Errorf("V44 外键检查发现 %d 个冲突", violations)
	}
	if err := recordVersion(ctx, tx); err != nil {
		return err
	}
	if err := tx.Commit(); err != nil {
		return fmt.Errorf("提交 V44 迁移: %w", err)
	}
	return nil
}
